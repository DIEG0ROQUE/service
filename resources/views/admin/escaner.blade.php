...
<head>
    ...
    <title>Validación de Tarjetones - ITO</title>
...
<body class="bg-gray-100 font-sans antialiased min-h-screen flex flex-col" x-data="colaboradorApp()">

    <div
        class="bg-indigo-700 text-white p-4 shadow-md flex justify-between items-center shrink-0 fixed top-0 w-full z-40">
        <div class="font-black tracking-widest uppercase text-sm flex items-center gap-2">
            <span>✅</span> Validación ITO
        </div>
        <a href="{{ route('login') }}"
            class="text-xs font-bold hover:text-indigo-200 uppercase bg-white/20 px-3 py-1.5 rounded-lg">Salir</a>
    </div>
    ...
    <div class="mt-16 p-4 w-full max-w-lg mx-auto flex-grow">
        ...
        <div x-show="tab === 'escaner'" x-transition>
            <div x-show="!mostrarDatos" class="w-full transition-all flex flex-col items-center justify-center">
                <div class="text-center mb-4">
                    <h2 class="text-xl font-black text-gray-800 uppercase">Escáner Oficial</h2>
                    <p class="text-gray-500 text-xs font-bold mt-1">Apunta al QR del tarjetón para sellar</p>
                </div>
                ...
                <div x-show="errorMsg" style="display: none;"
                    class="mt-4 w-full bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl text-center font-bold text-sm"
                    x-text="errorMsg"></div>
            </div>
        </div>
        ...
        <div x-show="tab === 'placas'" style="display: none;" x-transition>
            <div class="text-center mb-6">
                <h2 class="text-xl font-black text-gray-800 uppercase">Buscar por Placa</h2>
                <p class="text-gray-500 text-xs font-bold mt-1">Ingresa la placa para activar/desactivar</p>
            </div>
            ...
            <div class="bg-white p-6 rounded-[2rem] shadow-lg border border-gray-200 w-full mb-4">
                ...
                <button @click="buscarPlacaEnBD()" :disabled="buscando"
                    ...
                    <span x-text="buscando ? 'Buscando...' : '🔍 Buscar Vehículo'"></span>
                </button>
            </div>
        ...
        <div x-show="mostrarDatos" style="display: none;"
            ...
            <div class="p-4 text-center transition-all"
                ...
                <p class="text-white font-black text-xl tracking-widest uppercase"
                    x-text="datos.tarjeton.activo == 1 ? '✅ ESTATUS: ACTIVO' : '⚠️ PENDIENTE DE SELLO'"></p>
            </div>

            <div class="p-4 sm:p-6">
                <div class="flex items-center gap-3 mb-5 border-b border-gray-100 pb-5">
                    <div
                        class="w-16 h-20 bg-gray-100 rounded-lg overflow-hidden border-2 border-gray-300 shrink-0 flex items-center justify-center shadow-inner">
                    ...
                    <div class="flex-1 overflow-hidden">
                        <p class="text-[9px] font-black text-indigo-600 uppercase" x-text="datos.tipo"></p>
                        <p class="text-base font-black text-gray-800 uppercase leading-tight mb-1 truncate"
                            x-text="datos.nombre"></p>
                        <p class="text-[10px] font-bold text-gray-500 uppercase truncate" x-text="datos.adscripcion">
                        </p>
                        <p class="text-xs font-black text-gray-800 mt-1" x-text="'ID: ' + datos.identificador"></p>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-3 rounded-2xl grid grid-cols-2 gap-3 mb-6 border border-gray-100">
                    <div class="col-span-2 flex justify-between items-end border-b border-gray-200 pb-2">
                        <div>
                            ...
                            <p class="font-black text-indigo-700 uppercase text-xl leading-none"
                ...
                <button @click="cambiarEstatus()"
                    class="w-full py-4 rounded-xl font-black uppercase tracking-widest shadow-lg transition-all mb-3 flex items-center justify-center gap-2"
                    :class="datos.tarjeton.activo == 1 ? 'bg-red-600 hover:bg-red-700 text-white' :
                        'bg-green-600 hover:bg-green-700 text-white'">
                    <span
                        x-text="datos.tarjeton.activo == 1 ? '❌ Retirar Sello (Desactivar)' : '✅ Sellar Tarjetón (Activar)'"></span>
                </button>

                <button @click="limpiarDatos()"
                    class="w-full py-3 bg-gray-100 text-gray-600 rounded-xl font-bold uppercase hover:bg-gray-200 transition-all text-sm">Cerrar
                    / Buscar Otro</button>
            </div>
        </div>
    ...
    <div
        class="fixed bottom-0 w-full bg-white border-t border-gray-200 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] z-50 px-6 py-3 flex justify-center gap-12 items-center">
        <button @click="cambiarTab('escaner')" class="flex flex-col items-center gap-1 transition-all"
            :class="tab === 'escaner' ? 'text-indigo-700 scale-110' : 'text-gray-400'">
            <span class="text-2xl">📷</span>
            <span class="text-[9px] font-black uppercase tracking-widest">Escáner QR</span>
        </button>
        <button @click="cambiarTab('placas')" class="flex flex-col items-center gap-1 transition-all"
            :class="tab === 'placas' ? 'text-indigo-700 scale-110' : 'text-gray-400'">
            <span class="text-2xl">🔍</span>
            <span class="text-[9px] font-black uppercase tracking-widest">Buscar Placa</span>
        </button>
    </div>

    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('colaboradorApp', () => ({
                tab: 'escaner',
                mostrarDatos: false,
                buscando: false,
                placaSearch: '',
                ...
                cambiarTab(nuevaTab) {
                    ...
                    if (nuevaTab === 'escaner') {
                        ...
                        this.html5QrcodeScanner.clear(); // Apagar cámara para ahorrar batería
                    }
                },
                    ...
                    try {
                        let res = await fetch('/admin/escaner/buscar', {
                            ...
                            headers: {
                                ...
                                'ngrok-skip-browser-warning': 'true' // <--- SOLUCIÓN NGROK
                            },
                            body: JSON.stringify({
                                folio: decodedText
                            })
                        ...
                        this.errorMsg = "Error de red al leer el QR";
                    }
                },

                // CUANDO BUSCA POR PLACA
                async buscarPlacaEnBD() {
                    ...
                    try {
                        let res = await fetch('/admin/escaner/buscar-placa', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector(
                        ...
                        console.error("Error al buscar placa:", e);
                        this.errorMsg = "Error de conexión (Revisa tu terminal de Laravel)";
                    } finally {
                        this.buscando = false;
                    }
                },

                // PARA ACTIVAR / DESACTIVAR EL TARJETÓN
                async cambiarEstatus() {
                    try {
                        ...
                        console.error("Error en cambio de estatus:", e);
